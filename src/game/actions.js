...
export function startRecruitAction(s, cardId) {
...
export function drawCubeAction(s) {
  if (s.gameResult) return s;
  const p = getActivePlayer(s);
...
export function requestHelpAction(s) {
  if (s.gameResult) return s;
  const p = getActivePlayer(s);
...
export function resolveCombatAction(s) {
  if (s.gameResult) return s;
  const p = getActivePlayer(s);
...
export function forfeitCombatAction(s) {
  if (s.gameResult) return s;
  const p = getActivePlayer(s);
  if (!p.action || p.action.type !== 'combat') return s;
  const { targetId } = p.action;
...
export function cancelActionAction(s) {
  const p = getActivePlayer(s);
  if (!p.action) return s;
...
export function endDayAction(s) {
  if (s.gameResult) return s;
  const p = getActivePlayer(s);
...
export function dropCubeAction(s, cardId, cubeIndex) {
...
export function discardFoodAction(s, cubeIndex) {
  if (s.phase !== 'dusk') return s;
  const p = getActivePlayer(s);
  if (cubeIndex < 0 || cubeIndex >= p.band.length) return s;
...
export function returnCubeToBagAction(s, cardId, slotIndex) {
...
export function endNightAction(s) {
  if (s.phase !== 'night') return s;
  return advanceNight(s);
}

export function startFortressCombatAction(s, fortressId) {
  if (s.gameResult) return s;
  const p = getActivePlayer(s);
  if (p.action || s.phase !== 'day') return s;
